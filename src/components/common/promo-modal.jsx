...
export default function PromoModal() {
	const [isOpen, setIsOpen] = useState(false)
	const [isLoading, setIsLoading] = useState(false)
	const [isAgreed, setIsAgreed] = useState(true) // true - галка проставлена по умолчанию
	const [formData, setFormData] = useState({
		name: '',
		phone: '',
	})
		...
		try {
			const response = await fetch('/api/submit-promo', {
				...
				body: JSON.stringify({
					name: formData.name,
					phone: formData.phone,
					agreed: isAgreed, // Отправляем информацию о согласии
				}),
			})
			...
			setIsAgreed(true) // Сбрасываем на значение по умолчанию
			...
			<div className='relative bg-white rounded-lg shadow-xl w-full max-w-md mx-4 transform transition-all duration-300 ease-in-out scale-100 opacity-100'>
				<div className='p-6'>
					...
					<form onSubmit={handleSubmit} className='space-y-4'>
						...
						<div className='flex items-start'>
							<div className='flex items-center h-5'>
								<input
									...
									disabled={isLoading}
									className='w-4 h-4 text-(--accent-color) border-gray-300 rounded focus:ring-(--accent-color) cursor-pointer'
									required // Делаем чекбокс обязательным
								/>
							</div>
							<div className='ml-3 text-sm'>
								...
								>
									...
									<a
										...
										onClick={e => e.stopPropagation()} // Предотвращаем закрытие модалки при клике на ссылку
									>
						...
						<button
							type='submit'
							disabled={isLoading || !isAgreed} // Блокируем кнопку если нет согласия
							className={`w-full bg-(--accent-color) hover:bg-green-700 text-white font-bold py-3 px-4 rounded-md transition-all duration-200 transform hover:scale-105 hover:rotate-1 ${
								isLoading || !isAgreed
									? 'opacity-50 cursor-not-allowed hover:scale-100 hover:rotate-0'
									: ''
